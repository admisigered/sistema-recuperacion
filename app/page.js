...
export default function SistemaSIGERED() {
  ...
  // --- 1. LÓGICA DE NEGOCIO (REGLAS K, L, P, AB) ---
  const getEtapaEstado = useCallback((doc) => {
    ...
    if (doc.cargado_sisged || colL === 'SI SE VISUALIZA') return { etapa: 'CIERRE', estado: 'RECUPERADO', color: 'bg-green-100 text-green-700', border: 'border-green-500' };
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const valid = USUARIOS.find(u => u.user.toLowerCase() === loginData.user.toLowerCase() && u.pass === loginData.pass);
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden border border-white">
          <div className="bg-[#2563EB] p-12 text-center text-white"><h1 className="text-4xl font-black mb-2 tracking-tighter">SIGERED</h1><p className="text-xs uppercase font-bold tracking-widest opacity-80">Gestión de Recuperación</p></div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm h-auto min-h-[80px]">
          <div className="flex gap-2 mr-auto">
            ...
            {selectedIds.length > 0 && <button onClick={async () => { if(confirm("¿Eliminar?")) { await supabase.from('documentos').delete().in('id', selectedIds); setSelectedIds([]); fetchDocs(); } }} className="bg-red-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-red-200 flex items-center gap-2"><Trash2 size={14}/> Eliminar ({selectedIds.length})</button>}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            ...
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white border-slate-200 outline-none cursor-pointer shadow-sm" onChange={e => setFilters({...filters, origen: e.target.value})}><option value="">ORIGEN</option><option value="Interno">Interno</option><option value="Externo">Externo</option></select>
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white border-slate-200 outline-none cursor-pointer shadow-sm" onChange={e => setFilters({...filters, etapa: e.target.value})}><option value="">ETAPAS</option><option value="VERIFICACION">Verificación</option><option value="REQUERIMIENTO">Requerimiento</option><option value="SEGUIMIENTO">Seguimiento</option><option value="CIERRE">Cierre</option></select>
            ...
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white border-slate-200 cursor-pointer outline-none shadow-sm" onChange={e => setFilters({...filters, responsable: e.target.value})}><option value="">RESPONSABLE</option>{LISTA_RESPONSABLES.map(r => <option key={r} value={r}>{r}</option>)}</select>
                    ...
                    {['VERIFICACION', 'REQUERIMIENTO', 'SEGUIMIENTO', 'CIERRE'].map((etapa) => {
                      ...
                      return (
                        <div key={etapa} className="relative flex-1 flex flex-col items-center group">
                          ...
                          <div className="w-full bg-blue-600 rounded-t-xl shadow-lg" style={{ height: `${height}%`, minHeight: count > 0 ? '4px' : '0' }}></div>
              ...
              <div className="grid grid-cols-3 gap-6 font-sans">
                {USUARIOS.map(u => {
                  const uDocs = docs.filter(d => {
                    const m = d.responsable_verificacion === u.user.toUpperCase();
                    if (!m) return false;
                  ...
                  return (
                    <div key={u.user} className="bg-white border border-slate-100 p-8 rounded-[32px] shadow-sm space-y-4 hover:shadow-md transition-all">
                      ...
                      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600" style={{ width: `${pct}%` }}></div></div>
            ...
            <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden animate-in fade-in">
               <table className="w-full text-left border-collapse font-sans">
                <thead className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="p-6 pl-10 w-16 text-center border-r"><button onClick={() => { if(selectedIds.length === docs.length && docs.length > 0) setSelectedIds([]); else setSelectedIds(docs.map(d => d.id)); }}><Square size={22} className="text-slate-300 mx-auto"/></button></th>
                  ...
                  {docs.map(doc => {
                    ...
                    return (
                      <tr key={doc.id} className={`hover:bg-slate-50/80 transition-all ${isSelected ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <td className="p-6 pl-8"><p className="font-black text-slate-800 text-sm">{doc.cut}</p><p className="text-[10px] font-bold text-slate-400 uppercase mt-1 truncate max-w-[350px]">{doc.documento}</p></td>
      ...
      {/* --- MODAL DETALLES (SINCRO CORREGIDA + OBSERVACIONES) --- */}
      {editingDoc && (
              ...
              <div className="flex-1 p-14 overflow-y-auto bg-white font-sans">
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    <div className="space-y-3 font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Resp. Verificación</label>
                    ...
                    <div className="col-span-2 space-y-3 font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Observaciones (Col M)</label><textarea className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-xs shadow-inner" rows="3" value={editingDoc.observaciones || ''} onChange={e => setEditingDoc({...editingDoc, observaciones: e.target.value})}></textarea></div>
                  </div>
                )}
                {activeTab === 2 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300 font-sans">
                    <div className="space-y-3 font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Responsable Requerimiento (Col N)</label>
                    ...
                    <div className="space-y-3 font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Fecha Elaboración (Col O)</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner" value={editingDoc.fecha_elaboracion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_elaboracion: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">N° Documento Generado (Col P)</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner" value={editingDoc.numero_documento || ''} onChange={e => setEditingDoc({...editingDoc, numero_documento: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Fecha Notificación (Col Q)</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200 shadow-inner" value={editingDoc.fecha_notificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_notificacion: e.target.value})}/></div>
                    <div className="col-span-2 bg-blue-50 p-10 rounded-[40px] border border-blue-100 flex items-center justify-between shadow-inner shadow-slate-200"><div><p className="text-[10px] font-black text-blue-400 uppercase tracking-widest font-sans font-sans">Días Hábiles Transcurridos (Cálculo Automático)</p><p className="text-6xl font-black text-blue-600 mt-2 tracking-tighter">{calcularDiasHabiles(editingDoc.fecha_notificacion)}</p></div><Clock size={80} className="text-blue-200 opacity-50 font-sans"/></div>
                  </div>
                )}
                {activeTab === 3 && (
                  <div className="space-y-12 animate-in fade-in duration-300 font-sans">
                    <div className="bg-slate-50 p-10 rounded-[40px] space-y-6 border border-slate-200 font-sans font-sans"><h4 className="font-black text-xs uppercase text-slate-600 tracking-widest font-sans font-sans font-sans">Registrar Nuevo Seguimiento</h4><textarea id="s_obs" className="w-full p-6 rounded-[30px] border border-slate-100 bg-white text-sm outline-none shadow-inner font-medium font-sans font-sans" rows="3" placeholder="Detalles del contacto con el remitente..."></textarea>
                    ...
                    {seguimientos.map(s => (<div key={s.id} className="p-8 border border-slate-100 rounded-[35px] flex items-start gap-6 bg-white shadow-sm hover:shadow-md transition-shadow font-sans font-sans font-sans"><div className="bg-blue-100 p-4 rounded-2xl text-blue-600 shrink-0 shadow-inner font-sans font-sans font-sans font-sans"><MessageSquare size={24}/></div><div className="flex-1 font-sans font-sans"><div className="flex justify-between items-center mb-2 font-sans font-sans font-sans font-sans font-sans"><p className="text-xs font-black text-slate-800 uppercase tracking-widest font-sans font-sans font-sans font-sans">{s.responsable}</p><span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-full font-sans">{new Date(s.fecha).toLocaleDateString()}</span></div><p className="text-sm text-slate-500 font-medium italic font-sans font-sans font-sans">"{s.observaciones}"</p></div></div>))}
                  </div>
                )}
                {activeTab === 4 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300 font-sans">
                    <div className="col-span-2 bg-emerald-50 p-12 rounded-[45px] border border-emerald-100 flex items-center gap-8 shadow-inner font-sans font-sans"><input type="checkbox" className="w-12 h-12 accent-emerald-600 rounded-2xl shadow-sm cursor-pointer hover:scale-110 transition-transform font-sans" checked={editingDoc.cargado_sisged} onChange={e => setEditingDoc({...editingDoc, cargado_sisged: e.target.checked})}/><div><label className="font-black text-emerald-900 uppercase text-xs tracking-[0.2em] block mb-1 font-sans font-sans font-sans">Cargado en SISGED (Col AB)</label><p className="text-[10px] text-emerald-700 font-bold opacity-60 font-sans font-sans">Marque para finalizar documento como RECUPERADO.</p></div></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans">Estado Final Recuperación (Col AC)</label><select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase cursor-pointer shadow-inner" value={editingDoc.estado_final || 'PENDIENTE'} onChange={e => setEditingDoc({...editingDoc, estado_final: e.target.value})}><option value="PENDIENTE">PENDIENTE</option><option value="RECUPERADO">RECUPERADO</option><option value="RECONSTRUCCION">RECONSTRUCCION</option></select></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans font-sans">Oficina de Destino (Col AA)</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200" value={editingDoc.oficina_destino || ''} onChange={e => setEditingDoc({...editingDoc, oficina_destino: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans">Resp. Devolución (Col X)</label>
                    ...
                    <div className="col-span-2 space-y-3 font-sans font-sans font-sans font-sans font-sans font-sans"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans">Observaciones Finales (Col AD)</label><textarea className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-xs shadow-inner shadow-slate-200 font-sans font-sans" rows="3" value={editingDoc.observaciones_finales || ''} onChange={e => setEditingDoc({...editingDoc, observaciones_finales: e.target.value})}></textarea></div>
                  </div>
                )}
              </div>
            </div>
            <div className="p-10 bg-slate-50 border-t flex justify-end gap-6 shrink-0 font-sans font-sans font-sans font-sans font-sans"><button onClick={() => setEditingDoc(null)} className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-10 hover:text-slate-700 font-sans font-sans">Descartar</button><button onClick={async () => {
              ...
              }} className="bg-[#2563EB] text-white px-16 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all outline-none font-sans font-sans font-sans">Sincronizar Cambios</button></div>
          </div>
        </div>
      )}
      ...
      {/* --- MODAL NUEVO --- */}
      {isNewModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center z-[110] p-6 animate-in zoom-in duration-300 font-sans font-sans">
          <div className="bg-white rounded-[50px] w-full max-w-xl shadow-2xl p-12 space-y-10 border border-white relative font-sans font-sans font-sans font-sans">
            ...
            <div className="grid grid-cols-2 gap-6 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
              <input type="text" placeholder="CUT" className="w-full p-5 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans" id="n_cut" />
              <input type="text" placeholder="Documento" className="w-full p-5 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans" id="n_doc" />
              <input type="text" placeholder="Remitente" className="w-full p-5 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner col-span-2 shadow-slate-200 font-sans" id="n_rem" />
              ...
              <div className="relative col-span-2 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Phone size={16} className="absolute left-4 top-5 text-slate-300 font-sans"/><input type="text" placeholder="Celular" className="w-full p-5 pl-12 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans" id="n_cel" /></div>
              <div className="relative col-span-2 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Briefcase size={16} className="absolute left-4 top-5 text-slate-300 font-sans"/><input type="text" placeholder="Procedimiento TUPA" className="w-full p-5 pl-12 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans" id="n_proc" /></div>
              <select className="w-full p-5 bg-slate-50 border-none rounded-3xl font-black text-[10px] uppercase shadow-inner cursor-pointer font-sans" id="n_sede"><option value="SC">SEDE CENTRAL (SC)</option><option value="OD">ÓRGANO DESCONCENTRADO (OD)</option></select>
              <select className="w-full p-5 bg-slate-50 border-none rounded-3xl font-black text-[10px] uppercase shadow-inner cursor-pointer font-sans" id="n_origen"><option value="Externo">Externo</option><option value="Interno">Interno</option></select>
            </div>
            <button onClick={async () => {
