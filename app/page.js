...
export default function SistemaSIGERED() {
  ...
  // --- 1. LÓGICA DE NEGOCIO ANALÍTICA (REGLAS K, L, P, AB) ---
  const getEtapaEstado = useCallback((doc) => {
    ...
    // REGLA PRIORIDAD FINAL: ¿Se cargó al SISGED? (Col AB) -> CIERRE / RECUPERADO
    if (colAB === true || colAB === 'true' || colAB === 'SI') return { etapa: 'CIERRE', estado: 'RECUPERADO', color: 'bg-green-100 text-green-700', border: 'border-green-500' };
    ...
    if (colL === 'NO SE VISUALIZA') {
        if (origen === 'INTERNO') {
            // Documentos Internos: CIERRE / PENDIENTE
            return { etapa: 'CIERRE', estado: 'PENDIENTE', color: 'bg-red-100 text-red-700', border: 'border-red-500' };
        } else {
            // DOCUMENTOS EXTERNOS
            // Si todavía no se llena la columna P (numero_documento) -> REQUERIMIENTO / PENDIENTE
            if (!colP || colP === '' || colP === 'null') return { etapa: 'REQUERIMIENTO', estado: 'PENDIENTE', color: 'bg-red-100 text-red-700', border: 'border-red-500' };
            
            // Si se llenó Col P pero no hay seguimientos -> SEGUIMIENTO / PENDIENTE
            // Si se llenó algún seguimiento -> SEGUIMIENTO / EN PROCESO
    ...
    // REGLA PRIORIDAD 1: Columna K = PENDIENTE -> VERIFICACION / PENDIENTE
    if (colK === 'PENDIENTE') {
  ...
  // --- 4. IMPORTACIÓN A-AD (30 COLUMNAS CON LOGICA DE RESPONSABLE) ---
  ...
  // --- 5. SINCRONIZAR Y ACCIONES MASIVAS ---
  ...
  const handleBulkDelete = async () => {
    if (session.user.toUpperCase() !== 'ADMINISTRADOR') return alert("Permiso Denegado.");
    if (confirm(`¿Eliminar permanentemente ${selectedIds.length} registros?`)) {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "SIGERED_REPORT");
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <aside className="w-64 bg-[#1E293B] text-slate-400 flex flex-col fixed h-full z-20 shadow-2xl font-sans">
        ...
        <div className="p-6 border-t border-slate-800 flex items-center gap-3 bg-slate-900/50 font-sans">
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        {/* HEADER CON FILTROS SEGÚN IMAGEN */}
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm h-auto min-h-[80px] font-sans">
          <div className="flex gap-2 mr-auto font-sans">
            ...
            {selectedIds.length > 0 && <button onClick={handleBulkDelete} className="bg-red-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg animate-in fade-in"><Trash2 size={14}/> Eliminar Masivo ({selectedIds.length})</button>}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            ...
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white border-slate-200 cursor-pointer shadow-sm outline-none font-bold" onChange={e => setFilters({...filters, estado: e.target.value})}><option value="">ESTADO</option><option value="PENDIENTE">PENDIENTE</option><option value="EN PROCESO">EN PROCESO</option><option value="RECUPERADO">RECUPERADO</option></select>
            ...
            <div className="flex items-center gap-1 border border-slate-200 rounded-xl px-3 py-1.5 bg-slate-50"><Calendar size={12} className="text-slate-400"/><input type="date" className="bg-transparent text-[9px] font-bold outline-none" onChange={e => setFilters({...filters, fechaInicio: e.target.value})} title="Desde" /><span className="text-slate-300">-</span><input type="date" className="bg-transparent text-[9px] font-bold outline-none" onChange={e => setFilters({...filters, fechaFin: e.target.value})} title="Hasta" /></div>
          </div>
        </header>
        ...
        <div className="p-10 overflow-y-auto flex-1 font-sans">
          {view === 'dashboard' ? (
                    ...
                    {['VERIFICACION', 'REQUERIMIENTO', 'SEGUIMIENTO', 'CIERRE'].map((etapa) => {
                      ...
                      return (
                        <div key={etapa} className="relative flex-1 flex flex-col items-center group font-sans">
                          ...
                          <p className="absolute -bottom-8 text-[9px] font-black text-slate-400 uppercase text-center w-full tracking-tighter font-bold">{etapa}</p>
                        </div>
                      )
                    })}
                  </div>
                </div>
                <div className="col-span-6 bg-blue-600 p-10 rounded-[40px] text-white flex items-center justify-between shadow-2xl relative overflow-hidden font-sans">
                    <div className="relative z-10">
                        <h4 className="text-xs font-black uppercase opacity-70 tracking-widest mb-2 font-bold tracking-[0.2em]">Indicador Global de Éxito</h4>
                        ...
                        <p className="text-xs mt-4 opacity-80 font-bold leading-relaxed font-sans">Eficacia analítica de recuperación institucional.</p>
                    </div>
                    <CheckCircle2 size={120} className="opacity-10 absolute -right-4 -bottom-4"/>
                </div>
              </div>

              {/* AVANCE USUARIOS FILTRABLE */}
                ...
                {USUARIOS.map(u => {
                  ...
                  return (
                    <div key={u.user} className="bg-white border border-slate-100 p-8 rounded-[32px] shadow-sm space-y-4 hover:shadow-md transition-all shadow-slate-200">
                      ...
                      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600 transition-all duration-1000 shadow-lg shadow-blue-200" style={{ width: `${pct}%` }}></div></div>
            ...
            /* TABLA GESTIÓN CON SELECCIÓN MASIVA */
            <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden animate-in fade-in shadow-slate-100 font-sans">
               <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="p-6 pl-10 w-16 text-center border-r font-sans">
                    ...
                    <th className="p-6 text-center font-bold">Acciones</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={doc.id} className={`hover:bg-slate-50/80 transition-all ${isSelected ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <td className="p-6 pl-8">
                            <p className="font-black text-slate-800 text-sm font-sans font-bold">{doc.cut}</p>
                        ...
                        <td className="p-6 text-center"><div className="flex flex-col items-center gap-1 mx-auto"><span className="text-[9px] font-black bg-slate-200 text-slate-500 px-3 py-1 rounded-lg uppercase tracking-tighter font-bold">{status.etapa}</span><span className={`text-[10px] font-black px-4 py-1.5 rounded-xl border shadow-sm uppercase ${status.color}`}>{status.estado}</span></div></td>
                        <td className="p-6 text-center font-sans font-bold"><div className="flex items-center justify-center gap-3">
                            ...
                            {session.user === 'ADMINISTRADOR' && (<button onClick={() => handleDeleteIndividual(doc.id)} className="bg-white border-2 border-red-50 text-red-500 p-2.5 rounded-2xl hover:bg-red-600 hover:text-white transition-all shadow-sm shadow-red-100 font-sans"><Trash2 size={16}/></button>)}
                        </div></td>
                      </tr>
                    )
                  })}
              ...
              <div className="p-10 bg-slate-50 flex justify-between items-center border-t border-slate-100 font-sans shadow-inner shadow-slate-100 font-sans font-sans"><p className="text-xs font-black text-slate-400 uppercase tracking-widest">Página {page} • Registros: {totalDocs}</p>
                <div className="flex gap-4 font-sans"><button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center hover:bg-blue-600 hover:text-white shadow-sm disabled:opacity-20 transition-all shadow-lg shadow-slate-100"><ChevronLeft size={20}/></button><button onClick={() => setPage(p => p + 1)} disabled={page * 100 >= totalDocs} className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center hover:bg-blue-600 hover:text-white shadow-sm disabled:opacity-20 transition-all shadow-lg shadow-slate-100"><ChevronRight size={20}/></button></div>
              </div>
            </div>
          )}
        </div>
      ...
      {/* --- MODAL DETALLES TOTAL (SIN RECORTE - A-AD) --- */}
      {editingDoc && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-10 font-sans">
          <div className="bg-white rounded-[50px] w-full max-w-6xl h-[88vh] flex flex-col overflow-hidden shadow-2xl border border-white/20">
            <div className="p-10 bg-[#1E293B] text-white flex justify-between items-center shrink-0 font-sans">
              <div><h3 className="text-2xl font-black tracking-tight">{editingDoc.cut} • {editingDoc.documento}</h3><p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mt-2 tracking-[0.2em]">{editingDoc.origen} • {editingDoc.sede}</p></div>
              ...
              <div className="flex-1 p-14 overflow-y-auto bg-white font-sans">
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    <div className="space-y-3 font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Resp. Verificación (Col I)</label>
                      <select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase cursor-pointer shadow-inner shadow-slate-100" value={editingDoc.responsable_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, responsable_verificacion: e.target.value})}>
                        <option value="">SELECCIONE...</option>{LISTA_RESPONSABLES.map(r => <option key={r} value={r}>{r}</option>)}
                      </select>
                    </div>
                    <div className="space-y-3 font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Fecha Verificación (Col J)</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200 shadow-inner" value={editingDoc.fecha_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_verificacion: e.target.value})}/></div>
                    <div className="col-span-2 space-y-3 font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Estado Etapa (Col K)</label><select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase font-sans font-bold shadow-inner" value={editingDoc.estado_verificacion_k || ''} onChange={e => setEditingDoc({...editingDoc, estado_verificacion_k: e.target.value})}><option value="PENDIENTE">PENDIENTE</option><option value="VERIFICADO">VERIFICADO</option></select></div>
                    <div className="col-span-2 space-y-6 pt-6 text-center font-sans font-bold"><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] font-sans font-bold">Estado de Visualización (Col L)</p>
                      <div className="grid grid-cols-2 gap-8 font-sans font-bold font-bold"><button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'SI SE VISUALIZA'})} className={`p-10 rounded-[35px] border-2 font-black text-sm transition-all flex flex-col items-center gap-4 ${editingDoc.estado_visualizacion === 'SI SE VISUALIZA' ? 'border-green-600 bg-green-50 text-green-700 shadow-2xl shadow-green-900/10' : 'border-slate-50 bg-slate-50 text-slate-300 shadow-inner'}`}><CheckCircle2 size={32}/> SÍ SE VISUALIZA</button>
                        <button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'NO SE VISUALIZA'})} className={`p-10 rounded-[35px] border-2 font-black text-sm transition-all flex flex-col items-center gap-4 ${editingDoc.estado_visualizacion === 'NO SE VISUALIZA' ? 'border-red-600 bg-red-50 text-red-700 shadow-2xl shadow-red-900/10' : 'border-slate-50 bg-slate-50 text-slate-300 shadow-inner'}`}><AlertCircle size={32}/> NO SE VISUALIZA</button></div>
                    </div>
                    <div className="col-span-2 space-y-3 font-sans font-bold font-bold font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Observaciones Verificación (Col M)</label><textarea className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-xs shadow-inner shadow-slate-200 shadow-inner shadow-slate-200 shadow-slate-100 font-sans" rows="3" value={editingDoc.observaciones || ''} onChange={e => setEditingDoc({...editingDoc, observaciones: e.target.value})}></textarea></div>
                  </div>
                )}
                {activeTab === 2 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300 font-sans font-bold">
                    <div className="space-y-3 font-sans font-bold font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Responsable Requerimiento (Col N)</label>
                    ...
                    <div className="space-y-3 font-sans font-bold font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-bold">Fecha Elaboración (Col O)</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200" value={editingDoc.fecha_elaboracion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_elaboracion: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-bold font-bold font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-bold font-bold">N° Documento Generado (Col P)</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200 font-bold" value={editingDoc.numero_documento || ''} onChange={e => setEditingDoc({...editingDoc, numero_documento: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-bold font-bold font-bold font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans">Fecha Notificación (Col Q)</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200 shadow-inner" value={editingDoc.fecha_notificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_notificacion: e.target.value})}/></div>
                ...
                {activeTab === 3 && (
                  <div className="space-y-12 animate-in fade-in duration-300 font-sans">
                    <div className="bg-slate-50 p-10 rounded-[40px] space-y-6 border border-slate-200 font-sans font-sans font-sans"><h4 className="font-black text-xs uppercase text-slate-600 tracking-widest font-sans font-sans font-sans">Registrar Nuevo Seguimiento</h4><textarea id="s_obs" className="w-full p-6 rounded-[30px] border border-slate-100 bg-white text-sm outline-none shadow-inner font-medium shadow-slate-100 font-sans font-sans font-sans font-sans font-sans" rows="3" placeholder="Detalles del contacto con el remitente..."></textarea>
                    ...
                    {seguimientos.map(s => (<div key={s.id} className="p-8 border border-slate-100 rounded-[35px] flex items-start gap-6 bg-white shadow-sm hover:shadow-md transition-shadow font-sans font-sans"><div className="bg-blue-100 p-4 rounded-2xl text-blue-600 shrink-0 shadow-inner font-sans"><MessageSquare size={24}/></div><div className="flex-1 font-sans font-sans font-sans"><div className="flex justify-between items-center mb-2 font-sans font-sans font-sans font-sans"><p className="text-xs font-black text-slate-800 uppercase tracking-widest">{s.responsable}</p><span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-full font-sans font-sans font-sans font-sans">{new Date(s.fecha).toLocaleDateString()}</span></div><p className="text-sm text-slate-500 font-medium italic font-sans font-sans font-sans font-sans">"{s.observaciones}"</p></div></div>))}
                  </div>
                )}
                {activeTab === 4 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300 font-sans font-sans font-sans font-sans font-sans">
                    <div className="col-span-2 bg-emerald-50 p-12 rounded-[45px] border border-emerald-100 flex items-center gap-8 shadow-inner font-sans font-sans font-sans font-sans font-sans"><input type="checkbox" className="w-12 h-12 accent-emerald-600 rounded-2xl shadow-sm cursor-pointer hover:scale-110 transition-transform shadow-lg shadow-blue-900/10 font-sans shadow-lg font-bold" checked={editingDoc.cargado_sisged} onChange={e => setEditingDoc({...editingDoc, cargado_sisged: e.target.checked})}/><div><label className="font-black text-emerald-900 uppercase text-xs tracking-[0.2em] block mb-1 font-sans font-sans font-sans font-bold">Cargado en SISGED (Col AB)</label><p className="text-[10px] text-emerald-700 font-bold opacity-60 font-sans font-sans font-sans font-sans font-sans">Marque para finalizar documento como RECUPERADO.</p></div></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans">Estado Final de Recuperación (Col AC)</label><select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase cursor-pointer shadow-inner shadow-slate-100 font-bold" value={editingDoc.estado_final || 'PENDIENTE'} onChange={e => setEditingDoc({...editingDoc, estado_final: e.target.value})}><option value="PENDIENTE">PENDIENTE</option><option value="RECUPERADO">RECUPERADO</option><option value="RECONSTRUCCION">RECONSTRUCCION</option></select></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans font-sans">Oficina de Destino (Col AA)</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs shadow-inner shadow-slate-200 shadow-inner font-sans font-sans" value={editingDoc.oficina_destino || ''} onChange={e => setEditingDoc({...editingDoc, oficina_destino: e.target.value})}/></div>
                    <div className="space-y-3 font-sans font-sans font-sans font-sans font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans font-sans">Resp. Devolución (Col X)</label>
                    ...
                    <div className="col-span-2 space-y-3 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-bold"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 font-sans font-sans font-sans font-sans font-sans">Observaciones Finales (Col AD)</label><textarea className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-xs shadow-inner shadow-slate-200 shadow-inner shadow-slate-200 font-sans font-sans font-sans font-sans font-sans font-bold shadow-inner" rows="3" value={editingDoc.observaciones_finales || ''} onChange={e => setEditingDoc({...editingDoc, observaciones_finales: e.target.value})}></textarea></div>
                  </div>
                )}
              </div>
            </div>
            ...
            <button onClick={handleSyncChanges} className="bg-[#2563EB] text-white px-16 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl shadow-blue-900/40 tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all outline-none font-sans font-sans font-sans font-sans font-sans">SINCRONIZAR CAMBIOS</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center z-[110] p-6 animate-in zoom-in duration-300 font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
          <div className="bg-white rounded-[50px] w-full max-w-xl shadow-2xl p-12 space-y-10 border border-white relative font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
            ...
            <div className="grid grid-cols-2 gap-6 font-sans">
              <input type="text" placeholder="CUT" className="w-full p-5 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans font-sans" id="n_cut" />
              <input type="text" placeholder="Documento" className="w-full p-5 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans font-sans" id="n_doc" />
              ...
              <input type="date" className="w-full p-5 bg-slate-50 border-none rounded-3xl font-bold shadow-inner col-span-2 outline-none font-sans font-sans shadow-slate-100 font-sans font-sans font-sans font-sans" id="n_fecha" />
              <div className="relative col-span-2 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Phone size={16} className="absolute left-4 top-5 text-slate-300"/><input type="text" placeholder="Celular" className="w-full p-5 pl-12 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans" id="n_cel" /></div>
              <div className="relative col-span-2 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Briefcase size={16} className="absolute left-4 top-5 text-slate-300"/><input type="text" placeholder="Procedimiento TUPA" className="w-full p-5 pl-12 bg-slate-50 border-none rounded-3xl outline-none font-bold shadow-inner font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans" id="n_proc" /></div>
            ...
            }} className="w-full bg-[#2563EB] text-white py-6 rounded-[30px] font-black uppercase shadow-2xl tracking-[0.3em] hover:bg-blue-700 transition-all outline-none font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">Registrar Documento</button>
          </div>
        </div>
      )}
    </div>
