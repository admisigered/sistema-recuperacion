...
import { 
  ...
  CheckCircle2, AlertCircle, Trash2, Filter, X, CheckSquare, Square, Calendar, Phone, BookOpen, MessageSquare, BarChart3, Truck
} from 'lucide-react';
...
export default function SistemaSIGERED() {
  ...
  // --- CÁLCULO DE DÍAS HÁBILES (SIN SÁBADOS NI DOMINGOS) ---
  ...
  // --- LÓGICA DE ETAPA / ESTADO (ANÁLISIS DE REQUERIMIENTOS EXACTO) ---
  const getEtapaEstado = useCallback((doc) => {
    if (!doc) return { etapa: '-', estado: '-', color: 'bg-slate-100', border: 'border-slate-300' };
    if (doc.cargado_sisged || doc.estado_visualizacion === 'SI SE VISUALIZA') return { etapa: 'CIERRE', estado: 'RECUPERADO', color: 'bg-green-100 text-green-700', border: 'border-green-500' };
  ...
  const handleBulkDelete = async () => {
    if (confirm(`¿Eliminar ${selectedIds.length} registros permanentemente?`)) {
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      const batch = data.slice(1).map(row => ({
        sede: row[0], cut: String(row[1] || ''), documento: String(row[2] || ''), remitente: row[3], fecha_registro: row[4], origen: row[5],
        responsable_verificacion: row[8], fecha_verificacion: row[9], estado_visualizacion: row[11], numero_documento: String(row[15] || ''),
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm h-auto min-h-[80px]">
          <div className="flex gap-2">
            <button onClick={() => setIsNewModalOpen(true)} className="bg-[#2563EB] text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-blue-700 transition-all"><Plus size={14}/> Nuevo</button>
            ...
            {selectedIds.length > 0 && <button onClick={handleBulkDelete} className="bg-red-600 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 shadow-lg"><Trash2 size={14}/> Eliminar Masivo ({selectedIds.length})</button>}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            ...
            <select className="border rounded-xl p-2 text-[10px] font-black uppercase bg-white cursor-pointer" onChange={e => setFilters({...filters, sede: e.target.value})}><option value="">Sedes</option><option value="SC">SC</option><option value="OD">OD</option></select>
            <select className="border rounded-xl p-2 text-[10px] font-black uppercase bg-white cursor-pointer" onChange={e => setFilters({...filters, origen: e.target.value})}><option value="">Origen</option><option value="Interno">Interno</option><option value="Externo">Externo</option></select>
            <div className="flex items-center gap-1 border rounded-xl px-2 py-1 bg-slate-50">
                <Calendar size={12} className="text-slate-400"/><input type="date" className="bg-transparent text-[9px] font-bold outline-none" onChange={e => setFilters({...filters, fechaInicio: e.target.value})} title="Desde (Filtro Usuario)" /><span className="text-slate-300">-</span><input type="date" className="bg-transparent text-[9px] font-bold outline-none" onChange={e => setFilters({...filters, fechaFin: e.target.value})} title="Hasta (Filtro Usuario)" />
            </div>
          </div>
        </header>
          ...
          {view === 'dashboard' ? (
            <div className="space-y-12 animate-in fade-in">
              ...
              <div className="grid grid-cols-12 gap-8">
                    ...
                    {['VERIFICACION', 'REQUERIMIENTO', 'SEGUIMIENTO', 'CIERRE'].map((etapa) => {
                      ...
                      return (
                        <div key={etapa} className="relative flex-1 flex flex-col items-center group">
                          <div className="absolute bottom-[calc(100%+8px)] opacity-0 group-hover:opacity-100 transition-all bg-blue-600 text-white text-[11px] font-black px-3 py-1 rounded-lg shadow-xl translate-y-2 group-hover:translate-y-0 z-10">{count}</div>
                ...
                <div className="col-span-6 bg-blue-600 p-8 rounded-[30px] text-white flex items-center justify-between">
                    <div><h4 className="text-xs font-black uppercase opacity-80 mb-2">Indicador Global</h4><h3 className="text-5xl font-black">{totalDocs > 0 ? Math.round((docs.filter(d => getEtapaEstado(d).estado === 'RECUPERADO').length / totalDocs) * 100) : 0}%</h3></div>
                    <CheckCircle2 size={100} className="opacity-20"/>
                </div>
              </div>
                ...
                {USUARIOS.map(u => {
                  ...
                  return (
                    <div key={u.user} className="bg-white border p-8 rounded-[24px] shadow-sm space-y-4">
                      ...
                      <div className="h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600 transition-all" style={{ width: `${pct}%` }}></div></div>
            ...
            <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
               <table className="w-full text-left">
                <thead className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="p-5 pl-8 w-16 text-center"><button onClick={toggleSelectAll}>{selectedIds.length === docs.length && docs.length > 0 ? <CheckSquare size={20} className="text-blue-600 mx-auto"/> : <Square size={20} className="text-slate-300 mx-auto"/>}</button></th>
                  ...
                  {docs.map(doc => {
                    ...
                    return (
                      <tr key={doc.id} className={`hover:bg-slate-50/80 transition-all ${isSelected ? 'bg-blue-50/50' : ''}`}>
                        <td className="p-5 text-center"><button onClick={() => toggleSelectDoc(doc.id)}>{isSelected ? <CheckSquare size={20} className="text-blue-600 mx-auto"/> : <Square size={20} className="text-slate-200 mx-auto"/>}</button></td>
                        ...
                        <td className="p-5 text-center">
                           <div className="flex flex-col items-center gap-1 mx-auto"><span className="text-[9px] font-black bg-slate-200 text-slate-500 px-2 py-0.5 rounded uppercase">{status.etapa}</span><span className={`text-[10px] font-black px-4 py-1.5 rounded-xl border shadow-sm uppercase ${status.color}`}>{status.estado}</span></div>
                        </td>
                        <td className="p-5 text-center"><button onClick={() => { setEditingDoc(doc); setActiveTab(1); }} className="bg-white border-2 border-blue-50 text-blue-600 font-black text-[10px] px-4 py-2 rounded-xl hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest shadow-sm">Detalles</button></td>
                      </tr>
                    )
                  })}
                </tbody>
              </table>
              <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-between items-center"><p className="text-xs font-black text-slate-400 uppercase tracking-widest">Página {page} - {totalDocs} registros</p>
                <div className="flex gap-4"><button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="w-10 h-10 rounded-xl bg-white border flex items-center justify-center hover:bg-blue-600 hover:text-white disabled:opacity-30"><ChevronLeft size={18}/></button><button onClick={() => setPage(p => p + 1)} disabled={page * 100 >= totalDocs} className="w-10 h-10 rounded-xl bg-white border flex items-center justify-center hover:bg-blue-600 hover:text-white disabled:opacity-30"><ChevronRight size={18}/></button></div>
              </div>
            </div>
          )}
        </div>
      ...
      {/* --- MODAL DETALLES DEFINITIVO (INCLUYE TODA LA LÓGICA) --- */}
      {editingDoc && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[100] p-10">
          <div className="bg-white rounded-[40px] w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden shadow-2xl border border-white">
            <div className="p-8 bg-[#1E293B] text-white flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setEditingDoc(null)} className="w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center font-bold">✕</button>
            </div>
            <div className="flex flex-1 overflow-hidden font-sans">
              {/* TABS DEFINITIVOS */}
              <div className="w-72 bg-slate-50 border-r p-8 space-y-4 shrink-0">
                <button onClick={() => setActiveTab(1)} className={`w-full text-left p-6 rounded-[24px] font-black text-xs transition-all ${activeTab === 1 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-xl' : 'text-slate-400'}`}>1. VERIFICACIÓN</button>
                {editingDoc.origen === 'Externo' && (
                  <>
                    <button onClick={() => setActiveTab(2)} className={`w-full text-left p-6 rounded-[24px] font-black text-xs transition-all ${activeTab === 2 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-xl' : 'text-slate-400'}`}>2. REQUERIMIENTO</button>
                    <button onClick={() => setActiveTab(3)} className={`w-full text-left p-6 rounded-[24px] font-black text-xs transition-all ${activeTab === 3 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-xl' : 'text-slate-400'}`}>3. SEGUIMIENTO ({seguimientos.length})</button>
                  </>
                )}
                <button onClick={() => setActiveTab(4)} className={`w-full text-left p-6 rounded-[24px] font-black text-xs transition-all ${activeTab === 4 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-xl' : 'text-slate-400'}`}>4. CIERRE</button>
              </div>

              <div className="flex-1 p-12 overflow-y-auto bg-white">
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-10 animate-in fade-in">
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Responsable</label><select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs" value={editingDoc.responsable_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, responsable_verificacion: e.target.value})}><option value="">Seleccione...</option>{USUARIOS.map(u => <option key={u.user} value={u.user}>{u.user}</option>)}</select></div>
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs" value={editingDoc.fecha_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_verificacion: e.target.value})}/></div>
                    <div className="col-span-2 space-y-4 pt-4"><p className="text-[10px] font-black text-slate-400 uppercase text-center mb-4 tracking-[0.2em]">¿Se visualiza en el sistema?</p><div className="grid grid-cols-2 gap-6"><button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'SI SE VISUALIZA'})} className={`p-8 rounded-[32px] border-2 font-black transition-all ${editingDoc.estado_visualizacion === 'SI SE VISUALIZA' ? 'border-green-600 bg-green-50 text-green-700 shadow-lg' : 'border-slate-50 bg-slate-50 text-slate-300'}`}>SÍ SE VISUALIZA</button><button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'NO SE VISUALIZA'})} className={`p-8 rounded-[32px] border-2 font-black transition-all ${editingDoc.estado_visualizacion === 'NO SE VISUALIZA' ? 'border-red-600 bg-red-50 text-red-700 shadow-lg' : 'border-slate-50 bg-slate-50 text-slate-300'}`}>NO SE VISUALIZA</button></div></div>
                  </div>
                )}
                {activeTab === 2 && (
                  <div className="grid grid-cols-2 gap-10 animate-in fade-in">
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">N° Documento Generado</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs" value={editingDoc.numero_documento || ''} onChange={e => setEditingDoc({...editingDoc, numero_documento: e.target.value})}/></div>
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha Notificación</label><input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs" value={editingDoc.fecha_notificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_notificacion: e.target.value})}/></div>
                    <div className="bg-blue-50 p-8 rounded-[32px] border border-blue-100 flex items-center justify-between col-span-2"><div><p className="text-[10px] font-black text-blue-400 uppercase">Días Hábiles Transcurridos</p><p className="text-4xl font-black text-blue-600">{calcularDiasHabiles(editingDoc.fecha_notificacion)}</p></div><Clock size={48} className="text-blue-200"/></div>
                  </div>
                )}
                {activeTab === 3 && (
                  <div className="space-y-10 animate-in fade-in">
                    <div className="bg-slate-50 p-10 rounded-[40px] space-y-6 border border-slate-200">
                      ...
                      <textarea id="s_obs" className="w-full p-5 rounded-2xl border bg-white text-sm outline-none font-medium" rows="3" placeholder="Detalle del contacto..."></textarea>
                      <button onClick={async () => {
                        ...
                        if(!error) { await supabase.from('documentos').update({ ultimo_seguimiento: new Date().toISOString() }).eq('id', editingDoc.id); document.getElementById('s_obs').value = ''; alert("Registro Exitoso"); fetchDocs(); }
                      }} className="bg-blue-600 text-white font-black py-4 px-10 rounded-2xl text-xs uppercase shadow-xl tracking-widest">Grabar Registro</button>
                    </div>
                    {seguimientos.map(s => (<div key={s.id} className="p-6 border rounded-[28px] flex items-start gap-5 bg-white shadow-sm"><div className="bg-blue-100 p-4 rounded-2xl text-blue-600 shrink-0"><MessageSquare size={20}/></div><div><p className="text-xs font-black text-slate-800">{s.responsable} <span className="text-slate-400 font-normal ml-3">{new Date(s.fecha).toLocaleDateString()}</span></p><p className="text-sm text-slate-500 mt-2 leading-relaxed">{s.observaciones}</p></div></div>))}
                  </div>
                )}
                {activeTab === 4 && (
                  <div className="grid grid-cols-2 gap-10 animate-in fade-in">
                    <div className="col-span-2 bg-emerald-50 p-10 rounded-[40px] border border-emerald-100 flex items-center gap-6"><input type="checkbox" className="w-10 h-10 accent-emerald-600 rounded-2xl shadow-sm cursor-pointer" checked={editingDoc.cargado_sisged} onChange={e => setEditingDoc({...editingDoc, cargado_sisged: e.target.checked})}/><label className="font-black text-emerald-900 uppercase text-xs tracking-widest">¿Se cargó al portal SISGED? (MARCA SI)</label></div>
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estado Final de Recuperación</label><select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl font-black text-xs uppercase" value={editingDoc.estado_final || 'PENDIENTE'} onChange={e => setEditingDoc({...editingDoc, estado_final: e.target.value})}><option value="PENDIENTE">PENDIENTE</option><option value="RECUPERADO">RECUPERADO</option><option value="RECONSTRUCCION">RECONSTRUCCION</option></select></div>
                  </div>
                )}
              </div>
            </div>
            <div className="p-10 bg-slate-50 border-t flex justify-end gap-5 shrink-0"><button onClick={() => setEditingDoc(null)} className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-10">Descartar</button><button onClick={async () => {
              ...
              }} className="bg-[#2563EB] text-white px-16 py-5 rounded-[24px] font-black text-xs uppercase shadow-2xl tracking-widest">Sincronizar Cambios</button></div>
          </div>
        </div>
      )}
    </div>
