...
import { 
  ...
  Calendar, Phone, BookOpen, MessageSquare, BarChart3, Truck, Briefcase
} from 'lucide-react';
...
export default function SistemaSIGERED() {
  ...
  const getEtapaEstado = useCallback((doc) => {
    ...
    // ESTADO INICIAL: VERIFICACION / PENDIENTE
  ...
  // --- 3. ACCIONES DE DATOS (IMPORT, FETCH, DELETE) ---
    ...
    reader.onload = async (evt) => {
      ...
      const batch = data.slice(1).map(row => {
        ...
        return {
          ...
          fecha_verificacion: formatExcelDate(row[9]), estado_visualizacion: row[11], observaciones: row[12],
  ...
  const handleExport = (all = false) => {
    ...
    XLSX.writeFile(wb, `SIGERED_${all ? 'General' : 'Filtrado'}.xlsx`);
  };
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden border border-white">
          <div className="bg-[#2563EB] p-12 text-center text-white">
             ...
             <p className="text-xs font-bold uppercase tracking-widest opacity-80">Sistema de Recuperación</p>
          </div>
          <form onSubmit={handleLogin} className="p-10 space-y-6">
            ...
            <button type="submit" className="w-full bg-[#2563EB] text-white py-5 rounded-3xl font-black text-sm tracking-widest shadow-xl shadow-blue-200">INICIAR SESIÓN</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm h-auto min-h-[80px]">
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            <div className="relative"><Search size={14} className="absolute left-3 top-3 text-slate-400"/><input type="text" placeholder="CUT / Doc..." className="bg-slate-50 border-none rounded-xl pl-9 pr-4 py-2.5 text-xs w-32 outline-none focus:ring-2 focus:ring-blue-500" onChange={e => setFilters({...filters, search: e.target.value})}/></div>
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white outline-none cursor-pointer" onChange={e => setFilters({...filters, sede: e.target.value})}><option value="">Sedes</option><option value="SC">SC</option><option value="OD">OD</option></select>
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white outline-none cursor-pointer" onChange={e => setFilters({...filters, origen: e.target.value})}><option value="">Origen</option><option value="Interno">Interno</option><option value="Externo">Externo</option></select>
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-white outline-none cursor-pointer" onChange={e => setFilters({...filters, responsable: e.target.value})}><option value="">Responsable</option>{USUARIOS.map(u => <option key={u.user} value={u.user}>{u.user}</option>)}</select>
          ...
          {view === 'dashboard' ? (
            <div className="space-y-12 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-6 bg-blue-600 p-10 rounded-[40px] text-white flex items-center justify-between shadow-2xl shadow-blue-900/30">
                    <div>
                        <h4 className="text-xs font-black uppercase opacity-70 tracking-widest mb-2">Indicador de Eficacia</h4>
                        ...
                        <p className="text-xs mt-4 opacity-80 leading-relaxed font-bold">Porcentaje de éxito basado en el cierre de expedientes recuperados.</p>
                    </div>
            ...
            <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    return (
                      <tr key={doc.id} className={`hover:bg-slate-50/80 transition-all ${isSelected ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <td className="p-6 pl-8">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase truncate max-w-[350px] mt-1">{doc.documento}</p>
                        </td>
              ...
              <div className="p-10 bg-slate-50 flex justify-between items-center border-t border-slate-100">
                <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Página {page} • Registros: {totalDocs}</p>
                <div className="flex gap-4">
                    <button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center hover:bg-blue-600 hover:text-white shadow-sm disabled:opacity-20 transition-all"><ChevronLeft size={20}/></button>
                    <button onClick={() => setPage(p => p + 1)} disabled={page * 100 >= totalDocs} className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center hover:bg-blue-600 hover:text-white shadow-sm disabled:opacity-20 transition-all"><ChevronRight size={20}/></button>
                </div>
              </div>
            </div>
          )}
      ...
      {/* --- MODAL DETALLES TOTAL (SIN SIMPLIFICACIONES) --- */}
      {editingDoc && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-10">
          <div className="bg-white rounded-[50px] w-full max-w-6xl h-[88vh] flex flex-col overflow-hidden shadow-2xl border border-white/20">
            <div className="p-10 bg-[#1E293B] text-white flex justify-between items-center shrink-0">
              <div>
                ...
                <div className="flex gap-4 mt-2">
                    <span className="text-[10px] bg-blue-600 px-3 py-1 rounded-lg font-black uppercase tracking-widest">{editingDoc.origen}</span>
                    <span className="text-[10px] bg-slate-700 px-3 py-1 rounded-lg font-black uppercase tracking-widest">{editingDoc.sede}</span>
                </div>
              </div>
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="w-80 bg-slate-50 border-r p-10 space-y-4 shrink-0">
                <button onClick={() => setActiveTab(1)} className={`w-full text-left p-6 rounded-[30px] font-black text-xs transition-all flex items-center justify-between ${activeTab === 1 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-2xl' : 'text-slate-400'}`}>VERIFICACIÓN <CheckCircle2 size={16}/></button>
                {editingDoc.origen === 'Externo' && (
                  <>
                    <button onClick={() => setActiveTab(2)} className={`w-full text-left p-6 rounded-[30px] font-black text-xs transition-all flex items-center justify-between ${activeTab === 2 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-2xl' : 'text-slate-400'}`}>REQUERIMIENTO <Truck size={16}/></button>
                    <button onClick={() => setActiveTab(3)} className={`w-full text-left p-6 rounded-[30px] font-black text-xs transition-all flex items-center justify-between ${activeTab === 3 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-2xl' : 'text-slate-400'}`}>SEGUIMIENTO ({seguimientos.length}) <MessageSquare size={16}/></button>
                  </>
                )}
                <button onClick={() => setActiveTab(4)} className={`w-full text-left p-6 rounded-[30px] font-black text-xs transition-all flex items-center justify-between ${activeTab === 4 ? 'bg-white border-2 border-blue-600 text-blue-700 shadow-2xl' : 'text-slate-400'}`}>CIERRE <Save size={16}/></button>
              </div>

              <div className="flex-1 p-14 overflow-y-auto bg-white">
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Responsable</label>
                      <select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase" value={editingDoc.responsable_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, responsable_verificacion: e.target.value})}>
                        <option value="">Seleccione...</option>{USUARIOS.map(u => <option key={u.user} value={u.user}>{u.user}</option>)}
                      </select>
                    </div>
                    <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Fecha Verificación</label>
                      <input type="date" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs" value={editingDoc.fecha_verificacion || ''} onChange={e => setEditingDoc({...editingDoc, fecha_verificacion: e.target.value})}/>
                    </div>
                    <div className="col-span-2 space-y-6 pt-6">
                      <p className="text-[10px] font-black text-slate-400 uppercase text-center mb-4 tracking-[0.3em]">Estado de Visualización</p>
                      <div className="grid grid-cols-2 gap-8">
                        <button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'SI SE VISUALIZA'})} className={`p-10 rounded-[35px] border-2 font-black text-sm transition-all flex flex-col items-center gap-4 ${editingDoc.estado_visualizacion === 'SI SE VISUALIZA' ? 'border-green-600 bg-green-50 text-green-700 shadow-2xl shadow-green-900/10' : 'border-slate-50 bg-slate-50 text-slate-300'}`}><CheckCircle2 size={32}/> SÍ SE VISUALIZA</button>
                        <button onClick={() => setEditingDoc({...editingDoc, estado_visualizacion: 'NO SE VISUALIZA'})} className={`p-10 rounded-[35px] border-2 font-black text-sm transition-all flex flex-col items-center gap-4 ${editingDoc.estado_visualizacion === 'NO SE VISUALIZA' ? 'border-red-600 bg-red-50 text-red-700 shadow-2xl shadow-red-900/10' : 'border-slate-50 bg-slate-50 text-slate-300'}`}><AlertCircle size={32}/> NO SE VISUALIZA</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 2 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    ...
                    <div className="col-span-2 bg-blue-50 p-10 rounded-[40px] border border-blue-100 flex items-center justify-between">
                        <div><p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Días Hábiles Transcurridos</p><p className="text-6xl font-black text-blue-600 mt-2">{calcularDiasHabiles(editingDoc.fecha_notificacion)}</p></div>
                ...
                {activeTab === 3 && (
                  <div className="space-y-12 animate-in fade-in duration-300">
                    <div className="bg-slate-50 p-10 rounded-[40px] space-y-6 border border-slate-200">
                      ...
                      <textarea id="s_obs" className="w-full p-6 rounded-[30px] border border-slate-100 bg-white text-sm font-medium outline-none shadow-inner" rows="3" placeholder="Escriba aquí los detalles del contacto..."></textarea>
                    ...
                    <div className="space-y-8 font-sans">
                       {seguimientos.map(s => (
                         <div key={s.id} className="p-8 border border-slate-100 rounded-[35px] flex items-start gap-6 bg-white shadow-sm hover:shadow-md transition-shadow">
                           ...
                           <div className="flex-1"><div className="flex justify-between items-center mb-2"><p className="text-xs font-black text-slate-800 uppercase tracking-widest">{s.responsable}</p><span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-full">{new Date(s.fecha).toLocaleDateString()}</span></div><p className="text-sm text-slate-500 font-medium leading-relaxed italic">"{s.observaciones}"</p></div>
                         </div>
                       ))}
                    </div>
                  </div>
                ...
                {activeTab === 4 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    <div className="col-span-2 bg-emerald-50 p-12 rounded-[45px] border border-emerald-100 flex items-center gap-8 shadow-inner">
                       <input type="checkbox" className="w-12 h-12 accent-emerald-600 rounded-2xl shadow-sm cursor-pointer transition-transform hover:scale-110" checked={editingDoc.cargado_sisged} onChange={e => setEditingDoc({...editingDoc, cargado_sisged: e.target.checked})}/>
                       <div><label className="font-black text-emerald-900 uppercase text-xs tracking-[0.2em] block mb-1">Cargado en SISGED</label><p className="text-[10px] text-emerald-700 font-bold opacity-60">Seleccione si el documento ya figura en el portal institucional (Equivale a RECUPERADO)</p></div>
                    </div>
                    <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estado Final Recuperación</label>
                      <select className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs uppercase" value={editingDoc.estado_final || 'PENDIENTE'} onChange={e => setEditingDoc({...editingDoc, estado_final: e.target.value})}>
                    ...
                    <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Oficina de Destino</label><input type="text" className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-black text-xs" value={editingDoc.oficina_destino || ''} onChange={e => setEditingDoc({...editingDoc, oficina_destino: e.target.value})}/></div>
                    <div className="col-span-2 space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Observaciones Finales de Cierre</label><textarea className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-xs" rows="3" value={editingDoc.observaciones_finales || ''} onChange={e => setEditingDoc({...editingDoc, observaciones_finales: e.target.value})}></textarea></div>
                  </div>
                )}
              </div>
            </div>

            <div className="p-10 bg-slate-50 border-t flex justify-end gap-6 shrink-0">
              <button onClick={() => setEditingDoc(null)} className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-10 hover:text-slate-700 transition-colors">Descartar</button>
              <button onClick={async () => {
              ...
              }} className="bg-[#2563EB] text-white px-16 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl shadow-blue-900/40 tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all">Sincronizar Cambios</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isNewModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center z-[110] p-6 animate-in zoom-in duration-300">
          <div className="bg-white rounded-[50px] w-full max-w-xl shadow-2xl p-12 space-y-10 border border-white">
            <div className="text-center space-y-2"><h3 className="text-2xl font-black uppercase tracking-tighter text-slate-800">Nuevo Expediente</h3><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Ingrese los datos básicos del documento</p></div>
            <div className="grid grid-cols-2 gap-6">
              ...
              <select className="w-full p-5 bg-slate-50 border-none rounded-3xl font-black text-[10px] uppercase shadow-inner" id="n_sede"><option value="SC">SEDE CENTRAL (SC)</option><option value="OD">ÓRGANO DESCONCENTRADO (OD)</option></select>
              <select className="w-full p-5 bg-slate-50 border-none rounded-3xl font-black text-[10px] uppercase shadow-inner" id="n_origen"><option value="Externo">Externo</option><option value="Interno">Interno</option></select>
            </div>
            <button onClick={async () => {
              const doc = { 
              ...
              if (!error) { setIsNewModalOpen(false); fetchDocs(); } else alert("Error (Verifique si el CUT+Doc ya existe)");
            }} className="w-full bg-[#2563EB] text-white py-6 rounded-[30px] font-black uppercase shadow-2xl shadow-blue-200 tracking-[0.3em] hover:bg-blue-700 transition-all">Registrar Documento</button>
          </div>
        </div>
      )}
    </div>
