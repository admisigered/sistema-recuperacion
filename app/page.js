...
export default function SistemaSIGERED() {
  ...
  const getEtapaEstado = useCallback((doc) => {
    ...
    // REGLA 3: Etapa Verificación -> NO SE VISUALIZA
    if (doc.estado_visualizacion === 'NO SE VISUALIZA') {
        if (doc.origen === 'Interno') {
            return { etapa: '4°CIERRE', estado: 'PENDIENTE', color: 'bg-red-100 text-red-700', border: 'border-red-500' };
        } else {
            ...
            if (doc.ultimo_seguimiento) return { etapa: '3°SEGUIMIENTO', estado: 'EN PROCESO', color: 'bg-orange-100 text-orange-700', border: 'border-orange-500' };
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="p-10 space-y-5">
            ...
            <button type="submit" className="w-full bg-[#2563EB] text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">Iniciar Sesión</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <aside className="w-64 bg-[#1E293B] text-slate-400 flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => setView('dashboard')} className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl transition-all ${view === 'dashboard' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-900/40' : 'hover:bg-slate-800'}`}><LayoutDashboard size={18}/> Dashboard</button>
          <button onClick={() => setView('list')} className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl transition-all ${view === 'list' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-900/40' : 'hover:bg-slate-800'}`}><FileText size={18}/> Gestión</button>
          <button onClick={() => setView('reports')} className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl transition-all ${view === 'reports' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-900/40' : 'hover:bg-slate-800'}`}><Download size={18}/> Reportes</button>
        </nav>
        <div className="p-6 border-t border-slate-800 flex items-center gap-3 bg-slate-900/50">
          ...
          <button onClick={() => setSession(null)} className="hover:text-white transition-colors"><LogOut size={16}/></button>
        </div>
      </aside>
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm min-h-[80px]">
          <button onClick={() => setIsNewModalOpen(true)} className="bg-[#2563EB] text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-blue-700 transition-all shadow-sm"><Plus size={14}/> Nuevo</button>
          <label className="bg-white border border-slate-200 px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 cursor-pointer hover:bg-slate-50 transition-all shadow-sm"><Upload size={14}/> Importar <input type="file" className="hidden" onChange={handleImport}/></label>
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            ...
            <select className="bg-slate-50 border-none rounded-xl p-2 text-[10px] font-black uppercase outline-none" onChange={e => setFilters({...filters, estado: e.target.value})}><option value="">Estado</option><option value="PENDIENTE">PENDIENTE</option><option value="RECUPERADO">RECUPERADO</option><option value="RECONSTRUCCION">RECONSTRUCCION</option></select>
                  ...
                  {docs.map(doc => {
                    ...
                    return (
                      <tr key={doc.id} className="hover:bg-slate-50/80 transition-all">
                        ...
                        <td className="p-5"><div className="flex flex-col items-center gap-1"><span className="text-[9px] font-black bg-slate-200 text-slate-500 px-2 py-0.5 rounded uppercase tracking-tighter">{status.etapa}</span><span className={`text-[10px] font-black px-4 py-1.5 rounded-xl border shadow-sm uppercase ${status.color}`}>{status.estado}</span></div></td>
                        <td className="p-5 text-center">
                          <button onClick={() => { setEditingDoc(doc); setActiveTab(1); }} className="bg-white border-2 border-blue-50 text-blue-600 font-black text-[10px] px-4 py-2 rounded-xl shadow-sm hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest">Detalles</button>
                        </td>
                      </tr>
                    )
                  })}
      ...
      {/* --- MODAL DETALLES MULTI-ETAPA (FUNCIONAL) --- */}
      {editingDoc && (
          ...
          <div className="bg-white rounded-[40px] w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden shadow-2xl border border-white">
                ...
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-10 animate-in fade-in">
                    ...
                    <div className="col-span-2 space-y-4 pt-4">
                      <p className="text-[10px] font-black text-slate-400 uppercase text-center mb-4">¿Se visualiza correctamente en el sistema?</p>
                ...
                {activeTab === 3 && (
                  <div className="space-y-10 animate-in fade-in">
                    <div className="bg-slate-50 p-10 rounded-[40px] space-y-6 border border-slate-200">
                      ...
                      <textarea id="s_obs" className="w-full p-5 rounded-2xl border bg-white text-sm outline-none" rows="3" placeholder="Escriba aquí los detalles del contacto..."></textarea>
            ...
            <div className="p-10 bg-slate-50 border-t flex justify-end gap-5 shrink-0">
              ...
              <button onClick={async () => {
                ...
                if (!error) { alert('Datos Sincronizados'); setEditingDoc(null); fetchDocs(); }
              }} className="bg-blue-600 text-white px-16 py-5 rounded-[24px] font-black text-xs uppercase shadow-2xl shadow-blue-200">Guardar Cambios</button>
            </div>
          </div>
        </div>
      )}
