...
export default function SistemaSIGERED() {
  ...
  // --- FILTROS (Se adicionó ESTADO manteniendo el resto) ---
  const [filters, setFilters] = useState({ 
    search: '', 
    sede: '', 
    origen: '', 
    etapa: '', 
    estado: '', // Nuevo filtro adicionado
    responsable: '', 
    fechaInicio: '', 
    fechaFin: '' 
  });
  ...
  const getEtapaEstado = useCallback((doc) => {
    ...
    // Prioridad 1: Col AB (cargado_sisged) -> CIERRE / RECUPERADO
    if (doc.cargado_sisged) return { etapa: 'CIERRE', estado: 'RECUPERADO', color: 'bg-green-100 text-green-700', border: 'border-green-500' };

    // Prioridad 2: Col L (estado_visualizacion) SI SE VISUALIZA -> CIERRE / RECUPERADO
    ...
    if (doc.estado_visualizacion === 'NO SE VISUALIZA') {
        if (doc.origen === 'Interno') {
            ...
            // Externo: Evaluación Col P (numero_documento)
  ...
  const fetchDocs = useCallback(async () => {
    ...
    // Lógica DB para el Filtro de ESTADO Adicionado
    if (filters.estado === 'RECUPERADO') {
        query = query.or('cargado_sisged.eq.true,estado_visualizacion.eq.SI SE VISUALIZA,estado_final.eq.RECUPERADO');
  ...
  // --- 4. IMPORTACIÓN A-AD (30 COLUMNAS) ---
  ...
  // --- 5. DASHBOARD Y GRÁFICO BARRAS 50% ---
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-900 font-sans">
      ...
      <main className="ml-64 flex-1 flex flex-col h-screen overflow-hidden">
        {/* HEADER CON FILTROS SEGÚN IMAGEN + ADICIÓN ESTADO */}
        <header className="bg-white border-b p-4 flex flex-wrap items-center gap-3 sticky top-0 z-10 px-8 shadow-sm h-auto min-h-[80px]">
          <div className="flex gap-2 mr-auto">
            ...
            <label className="bg-white border border-slate-200 px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 cursor-pointer hover:bg-slate-50 shadow-sm"><Upload size={14}/> Importar <input type="file" className="hidden" onChange={handleImport}/></label>
          ...
          <div className="flex flex-wrap items-center gap-2 ml-auto">
            ...
            {/* --- FILTRO ESTADO ADICIONADO --- */}
            <select className="border rounded-xl p-2.5 text-[10px] font-black uppercase bg-blue-50 border-blue-100 text-blue-800 cursor-pointer outline-none" onChange={e => setFilters({...filters, estado: e.target.value})}>
        ...
        <div className="p-10 overflow-y-auto flex-1">
          {view === 'dashboard' ? (
            <div className="space-y-12 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-6 bg-blue-600 p-10 rounded-[40px] text-white flex items-center justify-between shadow-2xl">
                    <div><h4 className="text-xs font-black uppercase opacity-70 mb-2 tracking-widest">Indicador de Éxito</h4><h3 className="text-6xl font-black">{totalDocs > 0 ? Math.round((docs.filter(d => getEtapaEstado(d).estado === 'RECUPERADO').length / totalDocs) * 100) : 0}%</h3><p className="text-xs mt-4 opacity-80 font-bold leading-relaxed">Éxito basado en recuperación administrativa total.</p></div>
            ...
            /* TABLA DE GESTIÓN INTEGRAL CON SELECCIÓN */
                  ...
                  {docs.map(doc => {
                    ...
                    return (
                      <tr key={doc.id} className={`hover:bg-slate-50/80 transition-all ${isSelected ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <td className="p-6 text-center"><span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase ${doc.origen === 'Interno' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{doc.origen || 'EXTERNO'}</span></td>
                        ...
                        <td className="p-6 text-center"><button onClick={() => { setEditingDoc(doc); setActiveTab(1); }} className="bg-white border-2 border-blue-50 text-blue-600 font-black text-[10px] px-5 py-2.5 rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-sm">Detalles</button></td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-10 font-sans">
          <div className="bg-white rounded-[50px] w-full max-w-6xl h-[88vh] flex flex-col overflow-hidden shadow-2xl border border-white/20">
              ...
              <div className="flex-1 p-14 overflow-y-auto bg-white">
                {activeTab === 1 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    ...
                    <div className="col-span-2 space-y-6 pt-6 text-center"><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">¿Se visualiza en el sistema?</p>
                ...
                {activeTab === 2 && (
                  <div className="grid grid-cols-2 gap-12 animate-in fade-in duration-300">
                    ...
                    <div className="col-span-2 bg-blue-50 p-10 rounded-[40px] border border-blue-100 flex items-center justify-between shadow-inner"><div><p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Días Hábiles Transcurridos (Automático)</p><p className="text-6xl font-black text-blue-600 mt-2">{calcularDiasHabiles(editingDoc.fecha_notificacion)}</p></div><Clock size={80} className="text-blue-200 opacity-50"/></div>
                  </div>
                )}
                {activeTab === 3 && (
            ...
            <div className="p-10 bg-slate-50 border-t flex justify-end gap-6 shrink-0"><button onClick={() => setEditingDoc(null)} className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-10 hover:text-slate-700">Descartar</button><button onClick={async () => {
              ...
              }} className="bg-[#2563EB] text-white px-16 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl shadow-blue-900/40 tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all">Sincronizar Cambios</button></div>
          </div>
        </div>
      )}
      ...
      {isNewModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center z-[110] p-6 animate-in zoom-in duration-300">
          <div className="bg-white rounded-[50px] w-full max-w-xl shadow-2xl p-12 space-y-10 border border-white">
            ...
            }} className="w-full bg-[#2563EB] text-white py-6 rounded-[30px] font-black uppercase shadow-2xl shadow-blue-200 tracking-[0.3em] hover:scale-[1.02] transition-all">Registrar Documento</button>
          </div>
        </div>
      )}
    </div>
